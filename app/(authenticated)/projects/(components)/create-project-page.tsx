...
export function CreateProjectPage({ clients }: { clients: Client[] }) {
    ...
    const [profitMargin, setProfitMargin] = useState(57);
    ...
    const timeBurnPct =
        estimatedHours > 0 ? Math.min(100, (actualHours / estimatedHours) * 100) : 0;
    const startDt = startDate ? new Date(startDate) : null;
    const dueDt = dueDate ? new Date(dueDate) : null;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const daysRemaining =
        dueDt && startDt && isValid(dueDt) ? Math.max(0, differenceInDays(dueDt, today)) : 0;

    async function onSubmit(formData: FormData) {
        setIsSubmitting(true);
        formData.set("client_id", clientId);
        formData.set("title", title || "Untitled Project");
        ...
        <Card className="animate-fadein">
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <div className="flex items-center gap-3">
                    <Button variant="ghost" size="icon" asChild>
                        <Link href="/projects" aria-label="Cancel">
            ...
            <div className="border-b border-border px-6 py-5">
                ...
                <Typography variant="subtitle" className="text-muted-foreground mt-0.5">
                    Set up a new project with details, timeline and client.
                </Typography>
            </div>

            <form id="create-project-form" action={onSubmit} className="flex flex-1 min-h-0">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <Typography variant="bodySmall" className="text-muted-foreground mt-0.5 line-clamp-2">
                                            {section.description}
                                        </Typography>
                                    </div>
                                </button>
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                    ...
                                    <Field>
                                        ...
                                        <FieldDescription>
                                            A short summary visible on project cards and reports.
                                        </FieldDescription>
                                        <Textarea
                                            name="description"
                                            value={description}
                                    ...
                                    <Field>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            />
                                            <span className="size-6 rounded border border-border shrink-0" style={{ backgroundColor: color }} />
                                        </div>
                                    </Field>
                                    <Field>
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {PRIORITY_OPTIONS.map((opt) => (
                                                <Button
                                                    ...
                                                    size="sm"
                                                    className={cn(priority === opt.value && opt.value === "medium" && "bg-orange-500 hover:bg-orange-600")}
                                    ...
                                    <Field>
                                        ...
                                        <FieldDescription>Drag to set the current completion percentage.</FieldDescription>
                                        <p className="text-muted-foreground text-xs font-medium mb-2">Completion percentage</p>
                                        <div className="flex items-center gap-3">
                                            <input
                                                type="range"
                                                min={0}
                                            ...
                                            />
                                            <span className="text-primary font-medium text-sm w-10">{progress}%</span>
                                        </div>
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <Field>
                                            ...
                                            <div className="flex items-center border border-input rounded-md bg-input/20 overflow-hidden">
                                                <span className="text-muted-foreground pl-2.5 text-sm">{currency}</span>
                                                <Input
                                                    name="value"
                                                    type="number"
                                                    min={0}
                                    ...
                                    <div className="flex gap-6 pt-2">
                                        ...
                                        <span className="text-primary text-sm font-medium">Margin: {profitMargin}%</span>
                                    </div>
                                </FieldGroup>
                            </div>
                        )}
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                    ...
                                    <Field>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Progress value={timeBurnPct} className="flex-1 h-2" />
                                            <span className="text-muted-foreground text-sm whitespace-nowrap">
                                                {actualHours}h / {estimatedHours}h ({Math.round(timeBurnPct)}%)
                                            </span>
                                        </div>
                                    </Field>
                                </FieldGroup>
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                    ...
                                    <Field>
                                        ...
                                        <FieldDescription>Leave blank if the project is still in progress.</FieldDescription>
                                        <Input
                                            name="completed_date"
                                            type="date"
                                            value={completedDate}
                                            ...
                                            {startDt && dueDt && (
                                                <>
                                                    <div className="relative h-8 flex items-center">
                                                        ...
                                                        <span className="absolute left-0 text-[10px] text-muted-foreground" style={{ left: 0 }}>
                                                            {formatDate(startDate, "MMM d")}
                                                        </span>
                                                        <span className="absolute right-0 text-[10px] text-muted-foreground" style={{ right: 0 }}>
                            ...
                            <div className="max-w-2xl space-y-6">
                                <FieldGroup>
                                    <Field>
                                        ...
                                        <FieldDescription>The internal ID of the linked client record.</FieldDescription>
                                        <Input readOnly value={clientId || "—"} className="bg-muted/50" />
                                    </Field>
                                    <Field>
                                            ...
                                            <CardContent className="flex items-center gap-3 py-4">
                                                <Avatar size="lg">
                                                    <AvatarFallback className="bg-primary text-primary-foreground">
                                                        {(linkedClient.company || linkedClient.name).charAt(0).toUpperCase()}
                                                    </AvatarFallback>
                                                </Avatar>
                                                <div>
